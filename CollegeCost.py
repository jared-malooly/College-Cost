# Jared Malooly
# CollegeCost.py
# First project using big datasets, matplotlib, and excel data use
...
def plot_data_side(dates, public_data, private_data):
    '''
    Plots both stacked charts side by side for easier comparison between the two
    :param dates: 1963 - 2018
    :param public_data: Tuple containing housing, tuition, and board costs
    :param private_data: Tuple containing housing, tuition, and board costs
    :return:
    '''
    ...
    public_graph.set_ylim(0,43000)
    private_graph.title.set_text(private_data[3])
    private_graph.set_ylim(0,43000)

    y1 = [public_data[0], public_data[1], public_data[2]]
    y2 = [private_data[0], public_data[1], public_data[2]]

    private_graph.stackplot(x, y2, labels=['Tuition', 'Housing', 'Board'])
    public_graph.stackplot(x, y1, labels=['Tuition', 'Housing', 'Board'])

    public_graph.legend(loc='upper left')
    private_graph.legend(loc='upper left')
    public_graph.set_xticks(np.arange(dates[0], dates[-1] + 1, 17))
    private_graph.set_xticks(np.arange(dates[0], dates[-1] + 1, 17))
